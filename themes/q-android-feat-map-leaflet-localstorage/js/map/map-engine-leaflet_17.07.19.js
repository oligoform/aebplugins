...
function onEachFeature(feature, layer) {
 ...
 const secondUrl = 'https://am-eisernen-band.de/wp-json/locations/all';
 ...
 return fetch(secondUrl)
   ...
   return response.json();
	  ...
	  const flattenedArray = [].concat(...obj_locations); 
	
		   data.forEach(function(entry, index) {			  
	 				
			let location_id = entry.post_id;
		    let location_name = entry.location_name.replace(/["]/g, "'");
			
			let newArray = flattenedArray.filter(function (el) {  return el.ort_id == location_id });

			console.log(newArray[0]);	
			
			let veranstaltungen = newArray.length > 0 
				?  newArray.length > 1 
					? 'mehrere Veranstaltungen gefunden' 
					: `Angebote an diesem Ort: ${newArray[0]}`
				: 'keine Veranstaltungen gefunden';
			
		    let location_address = entry.location_address.replace(/["]/g, "'");
		    let popupContent = location_address + '<br />' +  entry.location_postcode + ' ' + entry.location_region;
						
			  obj[index] = `{"type":"Feature","properties": {"name": "${location_name}", "popupContent": "${veranstaltungen}" },
		   ...
		   geojsonFeature = '{"type": "FeatureCollection", "features": [' + geojsonFeature + ']}'
			...
			return geojsonFeature; 
				...
				L.geoJSON(obj, {
					onEachFeature: onEachFeature}).addTo( this.get('map_leaflet'));
		 }).catch(err => {
