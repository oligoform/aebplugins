    ...
    return Backbone.Model.extend({
        ...
        initMap: function () {
            if (!this.isMapActive() && $('#' + this.get('id')).length) {
                ...
                //                         let id = setTimeout(() => reject('Timeout'), 30000)

                //                         // console.log(getStoredData)

                //                         let ns = TemplateTags.getNetworkState(true);

                //                         if (ns === 'online') {

                //                         }

                //                         else return getStoredData;
                ...
                fetch(url).then(response => {
                    ...
                    let getStoredData = JSON.parse(localStorage.getItem("locationsJson"));
