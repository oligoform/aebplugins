            ...
            if (!this.isMapActive() && $('#' + this.get('id')).length) {
                ...
                fetch(url).then(response => {
                    ...
                    flattenedArray.forEach(function (entry, index) {
                        ...
                        let email = entry[arrayId].email !== "" ? '<br /> E-Mail: <a href="mailto:' + entry[arrayId].email + '">' + entry[arrayId].email + '</a><br/>' : '';
                        ...
                        //let olggeo = '<br><a title="Route von hier mit Google-Maps in neuem Fenster" class="btn btn-light btn-xs" target="_blank" href="https://maps.google.com/maps?daddr=' + entry[arrayId].locations.location_longitude + ',' + entry[arrayId].locations.location_latitude + '&amp;saddr=">Route</a><br>'; //olg test 20190820

                        let popupContent = `<div id='${location_id}'>${theTitle} <br /> ${location_address} <br /> ${location_postcode}${location_region}${barrierefrei}${email}${telefonnummer}${olggeo} <br>${frei} </div>`; //olg test 20190815
                        ...
                        $(".readmore").click(function (event) {
                            ...
                            locationsCached.forEach(function (entry, index) {
                                ...
                                if (arrayId == getId) {
                                    ...
                                    let email = entry[arrayId].email !== "" ? 'E-Mail: <a href="mailto:' + entry[arrayId].email + '">' + entry[arrayId].email + '</a><br/>' : '';
                                    ...
                                    if (link && link != '') {
                                        link = '<a href="' + link + '" target ="blank">' + linkname + '</a>';
                                    }


                                    modalContent.innerHTML = '<strong>' + theTitle + '</strong><br>';
